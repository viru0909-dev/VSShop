                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="flex items-center gap-3 pl-6 border-l border-white/10">
                                <div className="text-right hidden sm:block">
                                    ...
                                    <p className="text-xs text-indigo-400">{user.membership}</p>
                                </div>
                                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-gray-800 to-gray-700 border border-white/10 flex items-center justify-center text-xl shadow-inner">
                                    {user.avatar}
                                </div>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    ${idx === 0
                        ...
                        <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border border-indigo-500/20 rounded-2xl p-6 relative overflow-hidden group">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 blur-[40px] rounded-full group-hover:bg-indigo-500/20 transition-all duration-500"></div>
                            <h4 className="text-indigo-300 text-sm font-semibold mb-1">VSShop Credits</h4>
                            <p className="text-3xl font-bold text-white mb-4 tracking-tight">{user.credits}</p>
                            <button className="w-full py-2 bg-indigo-500 hover:bg-indigo-400 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-colors shadow-lg shadow-indigo-500/25">
                                Top Up
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-9 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                                { label: 'Total Orders', val: '24', icon: '🛍️', color: 'from-blue-500/20 to-cyan-500/20' },
                                { label: 'Pending Reviews', val: '2', icon: '⭐', color: 'from-orange-500/20 to-yellow-500/20' },
                                { label: 'Returns', val: '0', icon: '↩️', color: 'from-pink-500/20 to-rose-500/20' }
                            ].map((stat, i) => (
                        ...
                        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                                    Active Orders <span className="bg-indigo-500/20 text-indigo-300 text-xs px-2 py-1 rounded-full border border-indigo-500/20">Live Tracking</span>
                                </h2>
                                <button className="text-sm text-indigo-400 hover:text-indigo-300 transition-colors">View All &rarr;</button>
                            </div>

                            <div className="space-y-6">
                                {orders.map((order) => (
                                    <div key={order.id} className="group flex flex-col md:flex-row items-center gap-6 p-4 rounded-xl bg-white/5 border border-white/5 hover:border-indigo-500/30 transition-all duration-300 relative overflow-hidden">
                                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent translate-x-[-200%] group-hover:translate-x-[200%] transition-transform duration-1000"></div>

                                        <div className="w-16 h-16 rounded-lg bg-gray-800 flex items-center justify-center text-3xl shadow-lg">
                                            {order.image}
                                        </div>

                                        <div className="flex-1 w-full text-center md:text-left">
                                            <div className="flex justify-between items-start mb-1">
                                                <h3 className="font-bold text-lg text-white group-hover:text-indigo-300 transition-colors">{order.product}</h3>
                                                <span className="font-mono text-indigo-200">{order.price}</span>
                                            </div>
                                            ...
                                            <div className="relative pt-1">
                                                ...
                                                <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-gray-700">
                                                    <div style={{ width: `${order.progress}%` }} className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-gradient-to-r from-indigo-500 to-purple-500 animate-pulse"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-white/10 rounded-2xl p-6">
                                ...
                                <p className="text-gray-400 text-sm mb-4">Get 20% off on your next tech purchase with Premium membership.</p>
                                <div className="w-full h-32 rounded-lg bg-black/40 border border-dashed border-white/20 flex items-center justify-center">
                                    <span className="text-gray-500 text-sm font-mono tracking-widest">VS-PREMIUM-20</span>
                                </div>
                            </div>
                            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex items-center justify-between cursor-pointer hover:bg-white/10 transition-colors">
                                <div>
                                    <h3 className="font-bold text-white">Need Help?</h3>
                                    <p className="text-gray-400 text-sm">Chat with our AI support</p>
                                </div>
